...
def _update_work_experiences(resume_data, jd, llm):
    ...
    for i in range(len(work_experience)):
        work_experience[i] = llm.generateResponse(_get_work_exp_prompt(work_experience[i], jd["responsibilities"]))
...
def update_resume_data_based_on_jd(resume_data, jd):
    ...
    with open(f"{OUTPUT_FOLDER_PATH}{RESUME_DATA_JSON}", "w") as resume_data_file:
